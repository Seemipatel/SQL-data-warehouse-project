...
BEGIN
	...
	CASE
		WHEN UPPER(cst_material_status) = 'S' THEN 'Sinle'
		WHEN UPPER(cst_material_status) = 'M' THEN 'Married' 
		ELSE 'n/a'
	END cst_material_status, --- Normalize the marital status to 'Single' and 'Married', and handle unexpected values with 'n/a'
		CASE 
			WHEN UPPER(cst_gndr) = 'F' THEN 'Female'
			WHEN UPPER(cst_gndr) = 'M' THEN 'Male' 
			ELSE 'n/a'
		END cst_gndr, -- Normalize gender values to readable formats
		cst_create_date
	FROM (
		SELECT
		*,
 ...
 SET @start_time = GETDATE();
	...
	SELECT
		prd_id,
		prd_key,
		REPLACE(SUBSTRING(Prd_key, 1, 5), '-', '-') AS cat_id,  -- Extract category ID
		SUBSTRING(prd_key, 7, LEN(prd_key)) ASprd_key,          -- Extract product key
		prd_nm,
...
SET @end_time = GETDATE();
	...
	INSERT INTO silver.erp.loc_a101
	(cid,cntry)
	SELECT 
	REPLACE(cid, '-', '') cid,
	CASE
